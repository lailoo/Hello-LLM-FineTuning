...
![UniPELT实战指南：统一高效微调大模型的终极框架 - 架构图](../images/f105fbf20adc4963814be52d504e5056.png)
...
flowchart TB
    subgraph Transformer层
        direction TB
        MHA[多头注意力层] --> FFN[前馈网络层]
    end
    
    subgraph LoRA模块
        direction LR
        L_input[输入] --> L_lowrank[低秩矩阵ΔW] --> L_output[输出]
    end
    
    subgraph Prefix模块
        direction LR
        P_prompt[可学习Prefix向量] --> P_concat[拼接至Key/Value]
    end
    
    subgraph Adapter模块
        direction LR
        A_down[降维层] --> A_act[激活函数] --> A_up[升维层]
    end
    
    subgraph 门控网络
        G_L[gate-L] -->|控制LoRA| LoRA模块
        G_P[gate-P] -->|控制Prefix| Prefix模块
        G_A[gate-A] -->|控制Adapter| Adapter模块
    ...
    style LoRA模块 fill:#e6f7ff,stroke:#1890ff
    style Prefix模块 fill:#fff7e6,stroke:#fa8c16
    style Adapter模块 fill:#f6ffed,stroke:#52c41a
    style 门控网络 fill:#fafafa,stroke:#bfbfbf
```
...
```mermaid
sequenceDiagram
    participant Input as 输入向量
    participant SoftPrompt as Soft Prompt模块
    participant Attention as Attention层
    participant LoRA as LoRA模块
    participant FFN as 前馈网络FFN
    participant Adapter as Adapter模块
    participant Output as 输出向量
    Input->>SoftPrompt: 注入可学习前缀
    SoftPrompt->>Attention: 增强输入表征
    Attention->>LoRA: 注意力权重微调
    LoRA->>FFN: 传递注意力输出
    FFN->>Adapter: 插入瓶颈结构
    Adapter->>Output: 残差相加后输出
```
...
![柱状图展示在MRPC与QQP任务中，门控网络为LoRA、Prompt、Adapter三模块动态分配的激活权重，体现任务自适应调度能力](placeholder.png)
...
```mermaid
flowchart TB
    A[数据加载: MRPC/QQP] --> B[前向传播: 含Adapter/LoRA/Prefix门控]
    B --> C[损失计算: 二分类交叉熵]
    C --> D[反向传播: 梯度回传]
    D --> E[参数更新: 优化器步进]
    E -->|下一batch| A
...
![不同样本量下F1分数对比：UniPELT在小样本区段显著领先，且全程保持优势](../images/df8ea59d85c1482f9f249ed2ff24f647.png)
...
- **门控机制带来轻微延迟**  
  ...
  >
  > ![延迟随QPS变化曲线](https://example.com/uniplt-latency-qps.png) *(示意图：真实项目中绘制)*
