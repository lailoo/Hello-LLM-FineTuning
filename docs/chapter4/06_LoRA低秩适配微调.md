...
![全量微调、Adapter与LoRA三者对比：参数修改方式、参数量、推理延迟及适用范围](../images/ff90e62487124efbbc9290a7c08b5af9.png)
...
```mermaid
flowchart LR
    A[创建虚拟环境 lora_lab] --> B[安装 PyTorch]
    B --> C[安装 transformers 库]
    C --> D[安装 peft 库]
    D --> E[下载示例模型]
```
...
![柱状图对比原始模型、全量微调模型和LoRA微调模型在GLUE各子任务上的准确率表现](placeholder.png)
...
![折线图显示不同秩R值（2,4,8,16,32）在验证集上的性能表现，R=8时达到峰值](placeholder.png)
