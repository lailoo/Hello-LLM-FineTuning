...
![Soft Prompt实战指南：Prefix Tuning vs Prompt Tuning 轻量微调选型全解析 - 架构图](../images/fc89278e2e5847ad9c4603ff161a8f86.png)
...
```mermaid
flowchart TB
    A[人工设计Hard Prompt] --> B{是否可学习?}
    B -->|否| C[静态字符串注入\n依赖专家经验\n泛化性差]
    B -->|是| D[初始化Soft Prompt嵌入向量\n虚拟Token无真实词汇对应]
    D --> E[拼接至模型输入层]
    E --> F[反向传播优化嵌入参数]
    F --> G[自动适配目标任务\n提升泛化与稳定性]
```
...
flowchart TB
    subgraph 输入层["输入嵌入层"]
        I[原始输入Token Embedding]
    end
    subgraph 第1层Transformer["第1层 Transformer"]
        A1[自注意力模块]
        P1K[可学习前缀Key_1]
        P1V[可学习前缀Value_1]
        A1 -->|拼接| PKV1[修改后的K/V缓存]
        P1K --> PKV1
        P1V --> PKV1
    end
...
![模型规模与微调性能关系图，展示Prompt Tuning在超过10B参数后性能逼近全量微调](placeholder.png)
...
![Prefix Tuning与Prompt Tuning核心维度对比表，辅助选型决策](../images/b68549eae27541f7b3f62658163f29fa.png)
